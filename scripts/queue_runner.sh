...
do
   ...
   URL=$(jq '.url' <<< "$item")
   path_name=${ID//\"/}
   java -Xmx8G -Xms8G -jar gtfs-validator*SNAPSHOT_cli.jar --url $URL --output_base $OUTPUT_BASE/output/$path_name --validation_report_name latest.json --system_errors_report_name latest_errors.json
   exit 1
