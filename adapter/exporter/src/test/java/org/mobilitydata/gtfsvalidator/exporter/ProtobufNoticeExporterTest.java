...
//        verify(mockBuilder, times(1)).setAltEntityId(ArgumentMatchers.eq(null));
    ...
    void exportDecreasingStopTimeDistanceErrorNoticeShouldMapToCsvProblemAndWriteToStream() throws IOException {
        ...
        ProtobufNoticeExporter underTest = new ProtobufNoticeExporter(mockBuilder, mockStreamGenerator);
        underTest.export(new DecreasingStopTimeDistanceErrorNotice("12350", 2, 5f,
                22, 3f));
        ...
        verify(mockBuilder, times(1))
                .setOtherCsvKeyName(ArgumentMatchers.eq("2"));
        verify(mockBuilder, times(1))
                .setValue(ArgumentMatchers.eq("5.0"));
        verify(mockBuilder, times(1))
                .setAltEntityName(ArgumentMatchers.eq("3.0"));
        verify(mockBuilder, times(1))
                .setParentEntityName(ArgumentMatchers.eq("22"));
        verify(mockBuilder, times(1)).build();
        verify(mockProblem, times(1)).writeTo(ArgumentMatchers.eq(mockStream));
    }

    @Test
    void exportDecreasingStopTimeDistanceWarningNoticeShouldMapToCsvProblemAndWriteToStream() throws IOException {
        GtfsValidationOutputProto.GtfsProblem.Builder mockBuilder =
                mock(GtfsValidationOutputProto.GtfsProblem.Builder.class, RETURNS_SELF);
        ...
        ProtobufNoticeExporter underTest = new ProtobufNoticeExporter(mockBuilder, mockStreamGenerator);
        underTest.export(new DecreasingStopTimeDistanceWarningNotice("12350", 2, 5f,
                22));
