 ...
 */
public class ValidateRouteLongNameDoesNotContainShortName {
     ...
     */
    public ValidateRouteLongNameDoesNotContainShortName(final GtfsDataRepository dataRepo,
    ...
    public void execute() {
        ...
        routes.stream()
                .filter(route -> route.getRouteLongName().contains(route.getRouteShortName()))
                .forEach(route -> {
                    if (route.getRouteLongName().equals(route.getRouteShortName())) {
                        ...
                        resultRepo.addNotice(new RouteShortNameTooLongNotice("route.txt",
