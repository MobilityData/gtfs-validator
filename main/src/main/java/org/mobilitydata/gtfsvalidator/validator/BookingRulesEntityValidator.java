...
public class BookingRulesEntityValidator extends SingleEntityValidator<GtfsBookingRules> {
  ...
  public static class MissingPriorNoticeLastDayNotice extends ValidationNotice {
    /** The row number of the faulty record. */
    private final int csvRowNumber;
  ...
  /**
   * The `prior_notice_last_time` is required when booking_type=2 (prior day booking) is specified
   * in booking_rules.txt.
   */
  @GtfsValidationNotice(
      severity = SeverityLevel.ERROR,
      files = @GtfsValidationNotice.FileRefs(GtfsBookingRulesSchema.class))
  public static class MissingPriorNoticeLastTimeNotice extends ValidationNotice {
    /** The row number of the faulty record. */
    private final int csvRowNumber;
