...
| [`InvalidFrequencyBasedTripNotice`](#InvalidFrequencyBasedTripNotice)                                           	| Inconsistent `frequencies.exact_times` and `stop_times.timepoint` for a given trip.                                                                         	|
...
<a name="InvalidFrequencyBasedTripNotice"/>

#### InvalidFrequencyBasedTripNotice

Frequency-based trips with `frequencies.exact_times=0` should not have any `timepoint=1` values in `stop_times.txt`. The field should either be `0` or blank.

##### References:
* [frequency.txt specification](http://gtfs.org/reference/static/#frequencytxt)
